        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-3 flex-shrink-0">
          ...
          <div className="flex items-center justify-between sm:space-x-4">
            <button
              ...
              title={`${bulletinData.showQRCodeOnPrint !== false ? 'Hide' : 'Show'} QR code on printed bulletin`}
            >
              <QrCode className="w-4 h-4 mr-2" />
              <span className="hidden sm:inline">{bulletinData.showQRCodeOnPrint !== false ? 'QR Code: ON' : 'QR Code: OFF'}</span>
              <span className="sm:hidden">{bulletinData.showQRCodeOnPrint !== false ? 'QR ON' : 'QR OFF'}</span>
            </button>
